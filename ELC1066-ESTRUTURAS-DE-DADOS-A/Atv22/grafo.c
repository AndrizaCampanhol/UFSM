...
Grafo * grafo_cria (int num_vertices_max)
{
    ...
    for (int i = 0; i < num_vertices_max; i++) {
        int *col = (int *) malloc(sizeof(int) * num_vertices_max);
        grafo->matriz[i] = col;
    }
    int i = 0;
    ...
    grafo->max_verti = num_vertices_max;
    return grafo;
}
...
void grafo_libera (Grafo * grafo){
    ...
    for (int i = 0; i < grafo->max_verti; i++) {
        free(grafo->matriz[i]);
    }
    free(grafo->matriz);
    free(grafo);
...
Vertice * grafo_cria_vertice(Grafo * grafo, const char nome[]){
    if (grafo->ultimo_ocupado == grafo->max_verti)
        return NULL;
