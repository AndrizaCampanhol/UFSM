...
int * inicializa_mapa(Grafo * grafo){
   int tam = grafo_qtd_vertices(grafo);
   int *mapa = (int*) malloc(sizeof(int)*tam);
...
void detecta_amizade (Grafo * grafo, Lista * lista_branca, Lista * lista_cinza, Lista * lista_negra, int * mapa){
...
void detecta_ciclos (Grafo * grafo){
    ...
    detecta_amizade(grafo, lista_branca, lista_cinza, lista_negra, mapa);
}
...
void detecta_ciclos_em_vertice (Grafo * grafo, Vertice * origem){
    ...
    while(lista_branca->primeiro_item->vertice != origem && lista_branca->primeiro_item->vertice != NULL){
        lista_branca->primeiro_item = lista_branca->primeiro_item->prox_item;
    }
