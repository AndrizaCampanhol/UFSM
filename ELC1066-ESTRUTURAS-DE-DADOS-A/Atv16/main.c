...
int conta_nos_maiores(No * no, int v){
    ...
    conta_nos_maiores(arv_busca_filho(no, 0), v);
    conta_nos_maiores(arv_busca_filho(no, 1), v);
    return cont;
}
...
int main()
{
   ...
   //int rem = arv_remover_no1(arv_busca_raiz(arv), valor);
   //if (rem)
   //    printf("\nNo removido: %d", valor);
   //else
   ...
   printf("Nos maiores que %d: %d ", valor3, cont);
    ...
    int val = 50;
   int rem2 = arv_remover_no2(arv_busca_raiz(arv), val);
   if (rem2)
        printf("\nNo removido: %d", val);
   else
        printf("\nNo nao removido: %d", val);
